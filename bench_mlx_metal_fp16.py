...
SOURCE = r"""
    ...
    for (uint step = 0; step < N_STEPS; step++) {
        ...
        // QKV projections: each lane (0..15) computes one output row of all 3.
        // Accumulators in fp32 to keep precision.
        if (lane < uint(EMBD)) {
            float qv = 0.0f, kv = 0.0f, vv = 0.0f;
            int row_off = int(lane) * EMBD;
            for (int j = 0; j < EMBD; j++) {
                float xj = float(tg_x[j]);
        ...
        if (lane < uint(EMBD)) {
            float v = 0.0f;
            int row_off = int(lane) * EMBD;
            for (int j = 0; j < EMBD; j++) v += float(W[OFF_WO + row_off + j]) * float(tg_attn_out[j]);
            tg_x[lane] = half(v + float(tg_xr[lane]));
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        // MLP fc1: each of 32 lanes does 2 rows.
        {
            ...
            tg_h[row + 0] = half(metal::max(a0, 0.0f));
            tg_h[row + 1] = half(metal::max(a1, 0.0f));
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        if (lane < uint(EMBD)) {
            ...
            for (int j = 0; j < MLP_H; j++) v += float(W[OFF_W2 + row_off + j]) * float(tg_h[j]);
            tg_x[lane] = half(v + float(tg_xr[lane]));
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        if (lane < uint(VOCAB)) {
            float v = 0.0f;
            int row_off = int(lane) * EMBD;
            for (int j = 0; j < EMBD; j++) v += float(W[OFF_LM + row_off + j]) * float(tg_x[j]);
...
def main():
    ...
    def dispatch(seeds_in):
        outs = KERNEL(
            ...
            output_dtypes=[mx.uint32, mx.uint32, mx.float16, mx.float16],
        )
    ...
    label = f"mlx fp16 (gpu metal kernel S={S} N={N})"
