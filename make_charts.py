...
# Also render the original upstream-only reference charts so the comparison
# can sit side-by-side in a fork README.
UPSTREAM = [r for r in RESULTS if r[0].startswith("M4 Max") or r[0].startswith("FPGA")]


def horizontal_bars_subset(results_subset, values_fn, title, xlabel, fname, figsize=(9.5, 4.4)):
    labels = [r[0] for r in results_subset]
    values = [values_fn(r) for r in results_subset]
    colors = [color_for(l) for l in labels]
    order = sorted(range(len(values)), key=lambda i: values[i])
    labels = [labels[i] for i in order]
    vals = [values[i] for i in order]
...
horizontal_bars_subset(
    UPSTREAM, lambda r: r[1],
    "upstream reference — M4 Max MacBook Pro vs FPGA",
    "tokens / second",
    "throughput_upstream.png",
)
horizontal_bars_subset(
    UPSTREAM, lambda r: r[1] / r[2],
    "upstream reference — perf-per-watt, M4 Max vs FPGA",
    "tokens / second / watt",
    "perf_per_watt_upstream.png",
)
