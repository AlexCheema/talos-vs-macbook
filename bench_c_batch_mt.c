// microGPT batched inference, multi-threaded NEON.
// Splits B streams across T threads. Each thread is fully independent
// (own state, own RNG, own KV cache) -- no synchronization during the
// compute loop, only at start/end of the timed window via pthread_barrier.
//
// build: clang -O3 -march=native -ffast-math bench_c_batch_mt.c -o bench_c_batch_mt -lpthread
// run:   ./bench_c_batch_mt BATCH_SIZE N_THREADS [N_STEPS] [WARMUP]
...
int main(int argc, char **argv) {
    ...
    printf("  c fp32+NEON (batch=%d t=%d)        %14.0f tok/sec\n", B, T, rate);
